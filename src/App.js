    ...
    const cakeData = [
      {
        temp: 100,
        pies: [
          { filling: "apple", score: 12 },
          { filling: "cherry", score: 12 }
        ]
      },
      {
        temp: 90,
        pies: [
          { filling: "lemon", score: 12 },
          { filling: "orange", score: 12 }
        ]
      }
    ];
    ...
    return (
      <div>
        <header>
          ...
          <b style={{ color: "black" }}>CarSelect</b>
          ...
          {messages.map((x, indx) => {
            return (
              <span className="messageBody" key={indx}>
                {x.type === "q" && (
                  <div className="message from"> {x.text}</div>
                )}
        ...
        <form onSubmit={this.send}>
          <input onChange={this.onChange} value={currentAns} type="text" />
        </form>
      </div>
    );
  }
